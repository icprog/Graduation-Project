                  ...
                  id3_length_t *newlength)
{
  ...
  *newlength += *newlength / 1000;
  ...
  compressed = aw_mem_alloc(*newlength);
  if (compressed) {
//    if (compress2(compressed, newlength, data, length,
//          Z_BEST_COMPRESSION) != Z_OK ||
//    *newlength >= length) {
//      aw_mem_free(compressed);
//      compressed = 0;
//    }
//    else {
//      id3_byte_t *resized;
//
//      resized = aw_mem_realloc(compressed, *newlength ? *newlength : 1);
//      if (resized)
//    compressed = resized;
//    } peace
  }
                ...
                id3_length_t newlength)
{
  ...
  decompressed = aw_mem_alloc(newlength ? newlength : 1);
  if (decompressed) {
    id3_length_t size;

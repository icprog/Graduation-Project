...
signed long query_tag(int iofile)
{
  ...
  if ((save_position = aw_lseek(iofile, 0, SEEK_CUR)) < 0)
    return 0;

  size = id3_tag_query(query, aw_read(iofile, query, sizeof(query)));

  if (aw_lseek(iofile, save_position, SEEK_SET) < 0)
    return 0;
...
struct id3_tag *read_tag(int iofile, id3_length_t size)
{
  ...
  data = aw_mem_alloc(size);
  if (data) {
    if (aw_read(iofile, data, size) == size)
...
int add_filetag(struct id3_file *file, struct filetag const *filetag)
{
  ...
  tags = aw_mem_realloc(file->tags, (file->ntags + 1) * sizeof(*tags));
  if (tags == 0)
    return -1;
...
struct id3_tag *add_tag(struct id3_file *file, id3_length_t length)
{
  ...
  location = aw_lseek(file->iofile, 0, SEEK_CUR);
  if (location == -1)
    return 0;
...
int search_tags(struct id3_file *file)
{
  ...
  if ((off = aw_lseek(file->iofile, 0, SEEK_CUR)) < 0)
    return -1;

  /* look for an ID3v1 tag */

  if (aw_lseek(file->iofile, -128, SEEK_END) >= 0) {
  ...
  aw_lseek(file->iofile, 0, SEEK_SET);
  ...
  if (size > 0) {
    ...
    while (tag && (frame = id3_tag_findframe(tag, "SEEK", 0))) {
      ...
      if (seek < 0 || aw_lseek(file->iofile, seek, SEEK_CUR) < 0)
    break;
  ...
  if (aw_lseek(file->iofile, ((file->flags & ID3_FILE_FLAG_ID3V1) ? -128 : 0) +
        -10, SEEK_END) >= 0) {
    size = query_tag(file->iofile);
    if (size < 0 && aw_lseek(file->iofile, size, SEEK_CUR) >= 0) {
      size = query_tag(file->iofile);
      if (size > 0)
    add_tag(file, size);
    }
  ...
  if (aw_lseek(file->iofile, off, SEEK_SET) < 0)
    return -1;
 ...
 */
static
struct id3_file *new_file(int iofile, enum id3_file_mode mode,
              char const *path)
{
  struct id3_file *file;

  file = aw_mem_alloc(sizeof(*file));
  if (file == 0)
    goto fail;
...
//  iofile = fopen(path, (mode == ID3_FILE_MODE_READWRITE) ? "r+b" : "rb");
  iofile = aw_open(path, (mode == ID3_FILE_MODE_READWRITE) ? O_RDWR : O_RDONLY, 0777);
  if (iofile < 0)
    return 0;
         ...
         id3_byte_t const *data, id3_length_t length)
{
  ...
  if (data) {
    ...
    if (aw_lseek(file->iofile, (file->flags & ID3_FILE_FLAG_ID3V1) ? -128 : 0,
          SEEK_END) < 0 ||
    (location = aw_lseek(file->iofile, 0, SEEK_CUR)) == -1 ||
    aw_write(file->iofile, data, length) != 1)
      return -1;
...
# if defined(HAVE_FTRUNCATE)
    ...
    if (aw_lseek(file->iofile, 0, SEEK_END) < 0)
      return -1;
         ...
         id3_byte_t const *data, id3_length_t length)
{
  ...
  if (data &&
    ...
    if (aw_lseek(file->iofile, file->tags[0].location, SEEK_SET) < 0 ||
    aw_write(file->iofile, data, length) != 1)
      return -1;

...
# include "../libid3tag/frame.h"
# include "../libid3tag/render.h"
# include "../libid3tag/ucs4.h"
# include "../libid3tag/latin1.h"
# include "../libid3tag/parse.h"
...
void id3_field_finish(union id3_field *field)
{
  ...
  case ID3_FIELD_TYPE_LATIN1FULL:
    if (field->latin1.ptr)
        aw_mem_free(field->latin1.ptr);
    break;

  case ID3_FIELD_TYPE_LATIN1LIST:
    for (i = 0; i < field->latin1list.nstrings; ++i)
        aw_mem_free(field->latin1list.strings[i]);

    if (field->latin1list.strings)
        aw_mem_free(field->latin1list.strings);
    break;

  case ID3_FIELD_TYPE_STRING:
  case ID3_FIELD_TYPE_STRINGFULL:
    if (field->string.ptr)
        aw_mem_free(field->string.ptr);
    break;

  case ID3_FIELD_TYPE_STRINGLIST:
    for (i = 0; i < field->stringlist.nstrings; ++i)
        aw_mem_free(field->stringlist.strings[i]);

    if (field->stringlist.strings)
        aw_mem_free(field->stringlist.strings);
    break;

  case ID3_FIELD_TYPE_INT32PLUS:
  case ID3_FIELD_TYPE_BINARYDATA:
    if (field->binary.data)
        aw_mem_free(field->binary.data);
    break;
  }
            ...
            id3_length_t length, enum id3_field_textencoding *encoding)
{
  ...
  case ID3_FIELD_TYPE_LATIN1LIST:
    ...
    strings = aw_mem_realloc(field->latin1list.strings,
  ...
  case ID3_FIELD_TYPE_STRINGLIST:
    ...
    strings = aw_mem_realloc(field->stringlist.strings,
             ...
             unsigned int length, id3_ucs4_t **ptrs)
{
  ...
  strings = aw_mem_alloc(length * sizeof(*strings));
  if (strings == 0)
    return -1;

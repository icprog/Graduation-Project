...
struct id3_frame *id3_frame_new(char const *id)
{
  ...
  frame = aw_mem_alloc(sizeof(*frame) + frametype->nfields * sizeof(*frame->fields));
  if (frame) {
    frame->id[0] = id[0];
    frame->id[1] = id[1];
    frame->id[2] = id[2];
                  ...
                  id3_length_t decoded_length)
{
  ...
  mem = aw_mem_alloc(length ? length : 1);
  if (mem == 0)
    goto fail;
